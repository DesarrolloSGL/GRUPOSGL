<section x-data="{ cart_open: false , cat_open: false , variant_selected:false}">
    {{-- CART NAV  --}}
    ...
    <section class="my-10 flex">
        <div class="bg-global-50/50 text-global-950 w-full max-w-7xl h-fit rounded-lg md:mx-5 xl:mx-auto p-1 sm:p-10">
            <h3 class="text-right font-bold tracking-wide text-xs my-1 lg:my-0">SKU{{$product->sku}}</h3>
            <div class="w-full mx-auto lg:flex">
                {{-- Images --}}
                <div class="mb-5 lg:mb-0">
                    <div class="mx-auto min-h-[300px] min-w-[300px] max-w-sm max-h-96 overflow-hidden rounded-md border border-gray-200 flex">
                        <img class="mx-auto rounded-xl my-auto" src="{{asset('storage/store/'.$product->idproduct.'/0'.$product['img_extension'])}}">
                    </div>
                    @if (
                        $product->category_idcategory == 156 ||
                        $product->category_idcategory == 157 ||
                        $product->category_idcategory == 158 ||
                        $product->category_idcategory == 159
                        )
                        <a href="{{$product->link}}" target="_blank">
                            <div class="w-fit mr-auto my-5 sm:px-10 md:px-10 lg:px-0">
                                <img class="w-32 border border-gray-200 p-1 rounded-lg" src="{{asset('images/Brands/Shein.jpg')}}" alt="">
                                <h3 class="text-center font-bold text-sm tracking-wide">Ver en SHEIN</h3>
                            </div>
                        </a>

                    @elseif ($product->category_idcategory == 160)
                        <a href="{{$product->link}}" target="_blank">
                            <div class="w-fit mr-auto my-5 sm:px-10 md:px-10 lg:px-0">
                                <img class="w-32 border border-gray-200 p-1 rounded-lg" src="{{asset('images/Brands/FashionNova.jpg')}}" alt="">
                                <h3 class="text-center font-bold text-sm tracking-wide">Ver en Fashion Nova</h3>
                            </div>
                        </a>
                    @elseif ($product->category_idcategory == 164)
                        <a href="{{$product->link}}" target="_blank">
                            <div class="w-fit mr-auto my-5 sm:px-10 md:px-10 lg:px-0">
                                <img class="w-32 border border-gray-200 p-1 rounded-lg" src="{{asset('images/Brands/aliexpress.png')}}" alt="">
                                <h3 class="text-center font-bold text-sm tracking-wide">Ver en AliExpress</h3>
                            </div>
                        </a>
                    @else
                        <a href="{{$product->link}}" target="_blank">
                            <div class="w-fit mr-auto my-5 sm:px-10 md:px-10 lg:px-0">
                                <img class="w-32 border border-gray-200 p-1 rounded-lg" src="{{asset('images/home/294695_amazon_icon.png')}}" alt="">
                                <h3 class="text-center font-bold text-sm tracking-wide">Ver en Amazon</h3>
                            </div>
                        </a>
                    @endif
                    <div class="sm:px-10 md:px-10 lg:px-0">
                        <a href="https://wa.me/50232220185" target="_blank">
                            <h3 class="text-left text-global-600 text-xs">*Tienes dudas sobre este producto, contacta a un asesor.<span class="underline text-global-500 font-bold">aquí</span></h3>
                        </a>
                    </div>
                </div>
                <div class="w-full sm:px-10 md:px-10 relative">
                    <h3 class="text-lg text-justify font-bold">{{$product->name}}</h3>
                    <h3 class="p-2 sm:p-0 lg:mb-32 text-justify min-h-[200px]">{{$product->description}}</h3>
                    <div class="lg:absolute lg:bottom-0 lg:right-0">
                        <div class="flex space-x-1">
                            @if($product->variant)
                                @foreach (explode(",", $product->variant) as $selection)
                                    <div  wire:click="selectVariant('{{$selection}}')"
                                    class="p-2 w-9 h-8 text-center
                                    cursor-pointer text-xs underline underline-offset-4 rounded-lg border-global-300 text-global
                                    font-bold hover:bg-global-200"
                                    x-on:click="variant_selected = '{{$selection}}'"
                                    :class="variant_selected == '{{$selection}}' ? 'bg-global-200' : ''">{{$selection}}</div>
                                @endforeach
                            @endif
                        </div>
                        @if($product->compare_price)
                            <div class="w-fit ml-auto mx-5 flex whitespace-nowrap">
                                <h3 class="text-xs py-1 opacity-90 rounded-lg text-global-800 font-bold px-1">
                        ...
                        @endif
                        <div class="w-fit ml-auto mx-5 whitespace-nowrap">
                            <h3 class="text-lg py-1 opacity-90 rounded-lg text-global-800 font-bold tracking-wider">
                        ...
                        <div class="w-52 ml-auto">
                            <button wire:loading wire:target="addCart({{$product->idproduct}})" type="button" class="my-5 flex items-center w-full px-3 py-1 bg-global-700 text-white rounded-lg">
                                <svg aria-hidden="true" class="w-6 h-6 mx-auto  animate-spin text-global-700 fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button wire:loading.remove wire:target="addCart({{$product->idproduct}})" wire:click='addCart({{$product->idproduct}})' type="button"
                            class="{{$product->variant && (!$variant)? 'pointer-events-none opacity-50':'' }} my-5 text-center rounded-lg px-3 py-1 w-full bg-global-700 text-white hover:bg-global-800 transition-all delay-75 whitespace-nowrap">
                                Añadir al Carrito
                            </button>
                        </div>
                    </div>
                            ...
                            <li class="flex py-6" wire:loading.class.delay = "opacity-50" wire:loading.class = "pointer-events-none" wire:target="addCart,addUnits,decreaseUnits,removeCart">
                                ...
                                <div>
                                    <div class="flex justify-between text-base font-medium text-gray-900">
                                        <div class="line-clamp-3 cursor-pointer">
    ...
    {{-- END CART MODAL --}}
    <section class="mb-10">
        <livewire:store.product-slider :category="$product->category_idcategory">
    </section>
</section>
