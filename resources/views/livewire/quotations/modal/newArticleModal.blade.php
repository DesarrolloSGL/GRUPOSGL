...
<div x-cloak class="relative z-10" x-show="openAddArticleModal" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
          <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
            <div>
                ...
                <table class="table-auto rounded-lg w-full mx-auto">
                    ...
                    <tbody>
                        @foreach ($extra_costs as $key=>$item)
                            <tr class="text-center">
                                ...
                                <td class="border p-1 text-base text-center">
                                    <div class="mx-auto h-10 w-10">
                                        <button wire:loading wire:target="miamiNewArticleAdd({{$item->idcourier_extra_cost}})" wire:key='NewArticleLoad_{{$item->idcourier_extra_cost}}' class="bg-indigo-500 text-white block w-full h-full rounded-lg font-bold tracking-wide font-sans transition-all delay-100 hover:bg-indigo-400">
                                        ...
                                        <button wire:loading.remove wire:target="miamiNewArticleAdd({{$item->idcourier_extra_cost}})" wire:key='NewArticle_{{$item->idcourier_extra_cost}}' wire:click='miamiNewArticleAdd({{$item->idcourier_extra_cost}})' class="bg-indigo-500 text-white w-full h-full rounded-lg font-bold tracking-wide font-sans transition-all delay-100 hover:bg-indigo-400">+</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                ...
                @if (!$extra_costs)
                    <div class="mx-auto w-10 my-5">
                        <div wire:loading wire:target="miamiNewArticleLoadData" class="bg-white block w-full h-full rounded-lg font-bold tracking-wide font-sans transition-all delay-100 hover:bg-indigo-400">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            <button x-on:click="openAddArticleModal = false" type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cerrar</button>
          </div>
        </div>
      </div>
    </div>
